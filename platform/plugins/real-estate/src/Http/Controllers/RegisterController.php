...
class RegisterController extends Controller
{
    ...
    public function showRegistrationFormDev()
    {
        if (! RealEstateHelper::isRegisterEnabled()) {
            abort(404);
        }
        ...
        if (view()->exists(Theme::getThemeNamespace() . '::views.real-estate.account.auth.register-developer')) {
            return Theme::scope('real-estate.account.auth.register-developer')->render();
        }

        return view('plugins/real-estate::account.auth.register-developer');
    }
    ...
    public function registerDev(CreateUserRequest $request, BaseHttpResponse $response,CreateUserService $service)
    {
    ...
    protected function validatorDev(array $data)
    {
        $rules = [
            'first_name' => 'required|max:120',
            'last_name' => 'required|max:120',
            'username' => 'required|max:60|min:2|unique:users,username',
