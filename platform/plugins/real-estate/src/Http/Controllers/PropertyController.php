...
class PropertyController extends BaseController
{
    ...
    {
        $approvalRequests = PropertyAccess::where('is_approved',0)->with('project','account')->paginate(20);
        
        return view('plugins/real-estate::account.papproval',compact('approvalRequests'));
    }

    public function approve(Request $request,BaseHttpResponse $response)
    {
        $propertyAccess = PropertyAccess::find($request['id']);
        $propertyAccess->update(['is_approved'=>1]);
        return $response->setMessage(__('Project Approved Successfull'));
    }
}
