...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(document).on('click', '.add-to-wishlist', function (e) {
      ...
      if (propertyId != null && propertyId != 0 && propertyId != undefined) {
        ...
        if (wishCookies == undefined || wishCookies == null || wishCookies == '') {
          ...
          arrWList.push(item);
          $(".add-to-wishlist[data-id=".concat(propertyId, "] i")).removeClass('far fa-heart').addClass('fas fa-heart');
          ...
          if (index === -1) {
            ...
            clearCookies(cookieName);
            setCookie(cookieName, JSON.stringify(arrWList), 60);
            $(".add-to-wishlist[data-id=".concat(propertyId, "] i")).removeClass('fas fa-heart').addClass('far fa-heart');
    ...
    $(document).on('click', '.remove-from-wishlist', function (e) {
      ...
      if (propertyId != null && propertyId != 0 && propertyId != undefined) {
        ...
        if (index != -1) {
          ...
          setCookie(cookieName, JSON.stringify(arrWList), 60);
          showSuccess(__('Removed from wishlist successfully!'));
          $(".wishlist-page .item[data-id=".concat(propertyId, "]")).closest('div').remove();
        }
      }
      var countWishlist = JSON.parse(getCookie(cookieName)).length;
      $('.wishlist-count').text(countWishlist);
    ...
    function checkWishlistInElement($el) {
      var parseCookie = JSON.parse(getCookie('wishlist') || '{}');
